...
int func4(int x) {
    int first = x<<16;
    int mask_1 = ~((0x1 << 31) >> 15);
    int last = x>>16 & mask_1;
    return first | last;
}
/* 
 ...
 */
int func9(int x) {
    //x+x+2=0, overflow if x is max and x+x+2==0
  return !(x+x+2) & !(!(x+1));
}
/* 
 * func10 - return -x 
...
int func11(int x, int y) {
    //overflow occurs if x,y are of same sign and x+y is of opposite sign
    //check if a and b have the same sign, if not then there is no overflow
    int z=x+y;
    int a=x>>31;
    int b=y>>31;
    int c=z>>31;
...
int func12(int x, int y) {
    ...
    int is_x = (x >> 31) & 1;
    int is_y = (y >> 31) & 1;
    int is_diff = ((diff + 1) >> 31) & 1;
    return ((!is_x) & (is_y)) | ((!((is_x) & (!is_y)))&(!is_diff)&!(!(diff+1)));
}
/*
...
int func13(int x, int y) {
    ...
    int is_y = y>>31;
    int min_int = 1<<31;
    int ss = !(is_y^is_x); //true if same sign
    int overflow = (is_x^is_xy) & (is_y^is_xy);        // checks for overflow
    int pos_overflow = ss & (!is_x);
    int sum = (((!ss) | !overflow)<<31)>>31 & xy;
    int max = ((pos_overflow & overflow)<<31)>>31 & ~min_int;
    int min = (((!pos_overflow) & overflow)<<31)>>31 & min_int;
    return sum + max + min;
}
